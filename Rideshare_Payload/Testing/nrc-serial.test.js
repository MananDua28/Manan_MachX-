...
const { createRideshareSerial } = require('../Firmware/Ground_Station/rideshare-serial');
const { crc16Ccitt } = require('../../CanSAT/Firmware/Ground_Station/cansat-hardware');
...
async function withMockedSerialModule(env, fn, options = {}) {
  ...
  const serialPath = require.resolve('../../Shared_Ground_Station/backend/serial');
  delete require.cache[serialPath];
  const serial = require('../../Shared_Ground_Station/backend/serial');
